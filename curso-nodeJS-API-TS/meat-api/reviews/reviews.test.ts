...
test('POST /REVIEWS/', async () => {
    ...
    return Promise.all([
        request(URL_TEST).post('/restaurants').set('Authorization',auth).send({ name : "Restaurante-Review" , menu : [] }) , 
        request(URL_TEST).post('/users').set('Authorization',auth).send({ name : "TEST" , email : "test@test.tes" , password : "123456" })
    ])
    .then(([restaurant,user]) => {
        ...
        return request(URL_TEST).post('/reviews').set('Authorization',auth).send({
...
test('GET / REVIEW BY ID' , async () => {
    ...
    return Promise.all([
        request(URL_TEST).post('/restaurants').set('Authorization',auth).send({ name : "Restaurante-Review1" , menu : [] }) , 
        request(URL_TEST).post('/users').set('Authorization',auth).send({ name : "TEST1" , email : "test1@test.tes" , password : "123456" })
    ])
    .then(([restaurant,user]) => {
    ...
    .then(response => {
        ...
        return request(URL_TEST).get(`/reviews/${response.body._id}`).set('Authorization',auth)
    })
